...
def create_gif_poster(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        poster = VideoFileClip(f"{BASE_DIR/instance.video.path}").save_frame(
            dest_path_poster, t=0.00
        )
        gif = (
            VideoFileClip(f"{BASE_DIR/instance.video.path}")
            .subclip(0, 2)
            .write_gif(dest_path_gif)
        )

        instance.gif.save(
            f"{dest_file_name_gif}", content=File(open(dest_path_gif, "rb"))
        )
        instance.poster.save(
            f"{dest_file_name_poster}", content=File(open(dest_path_poster, "rb"))
        )
